...
4. **Declared metadata.** A `template.yaml` (templates) or `pattern.yaml` (patterns) — see [docs/authoring-guide.md](./docs/authoring-guide.md) for the schema.
5. **Interface (patterns only).** Patterns declare their pins (inputs/outputs) so they're swappable/composable.
